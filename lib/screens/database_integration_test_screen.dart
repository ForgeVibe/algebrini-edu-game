...
class DatabaseIntegrationTestScreen extends StatefulWidget {
  ...
  @override
  State<DatabaseIntegrationTestScreen> createState() =>
      _DatabaseIntegrationTestScreenState();
}

class _DatabaseIntegrationTestScreenState
    extends State<DatabaseIntegrationTestScreen> {
  ...
  Future<void> _testDatabaseConnection() async {
    ...
    if (games.isNotEmpty) {
      _addTestResult(
          '✅ Database connection successful - found ${games.length} games');
    } else {
  ...
  Future<void> _testGameRegistry() async {
    ...
    if (registry.useDatabase) {
      _addTestResult(
          '✅ Game registry using database - ${games.length} games loaded');
    } else {
      _addTestResult(
          '⚠️ Game registry using hardcoded data - ${games.length} games loaded');
    }
  ...
  Future<void> _testSimpleEquationsGame() async {
    ...
    await Future.delayed(
        const Duration(milliseconds: 500)); // Wait for async loading
  ...
  Future<void> _testProgressSaving() async {
    ...
    final progress =
        await _gameDataService.getUserProgress(testUserId, testGameId);
